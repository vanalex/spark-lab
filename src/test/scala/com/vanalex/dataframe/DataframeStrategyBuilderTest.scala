...
class DataframeStrategyBuilderTest
    extends FunSpec
  ...
  describe("loads dataframe") {
    ...
    it("it loads dataframe given parquet path") {
      val dataframe = DataframeStrategyBuilder.build(
    ...
    it("it loads dataframe given csv path") {
      val dataframe = DataframeStrategyBuilder.build(
        "src/main/resources/retail-data/by-day/2011-12-09.csv",
        spark
      )
      assertNotNull(dataframe)
    }
    ...
    it("it loads dataframe given json path") {
      val dataframe = DataframeStrategyBuilder.build(
        "src/main/resources/flight/2015-summary.json",
        spark
      )
      assertNotNull(dataframe)
    }
